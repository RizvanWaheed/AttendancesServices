...
namespace AttendancesServices
{
    class EmployeeLogMapping : Common, IDisposable
    {
        ...
        private DataSet GetMachineAttendance()
        {
            ...
            DataSet AttendanceMachineDS = new DataSet();
            ...
            try
            {
                ...
                // Way 3
                using (MySqlCommand AttendanceMachineEmp = new MySqlCommand(AttendanceMachineQry, conn))
                {
                    using (MySqlDataAdapter AttendanceMachineDA = new MySqlDataAdapter(AttendanceMachineEmp))
                    {
        ...
        private DataTable GetEmployeeLogs(string employee_id, string day)
        {
            ...
            DataTable EmployeeLogsDT = new DataTable();
            
            Console.Write(" {0}\n", SelectEmployeeLogs);
            try
            {
                using (MySqlCommand EmployeeLogsCmd = new MySqlCommand(SelectEmployeeLogs, conn))
                {
                    using (MySqlDataAdapter EmployeeLogsDA = new MySqlDataAdapter(EmployeeLogsCmd))
                    {
