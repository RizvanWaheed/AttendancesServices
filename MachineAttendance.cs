...
namespace AttendancesServices
{
    sealed class MachineAttendance : CommonQueries, IDisposable //,  // : IDisposable
    {
        ...
        private Dictionary<string, string> GetUserEmployee(string code)
        {
            // DateTime cardTime = Convert.ToDateTime(dateValue);

            string employyeeUserQry = "SELECT `tbl_users`.`role_id`, `tbl_users`.`login`, `tbl_employees`.`id`, `tbl_employees`.`sap_code`, `tbl_employees`.`full_name`, `tbl_setups`.`slug` ";
