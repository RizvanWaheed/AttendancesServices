...
{
    sealed class AttendancesShifts : CommonQueries, IDisposable
    {
        ...
        public void GetAttendanceShiftWise()
        {
            Console.WriteLine("  Local Date: {0}", localDate);
            Console.WriteLine("  Local Two Month Date: {0}", localMonthDate);

            DataSet shitDS = GetShifts();
            foreach (DataRow shiftRow in shitDS.Tables["Shifts"].Rows)
            {
        ...
        private DataSet GetShifts()
        {
            string shiftUsrEmpQry = " SELECT `asm_id`, MIN(`dates`) as dt ";
            shiftUsrEmpQry += " FROM `tbl_attendances_shifts`";
            shiftUsrEmpQry += " WHERE `status` = 1  " +
                " AND (" +
                "     (`created_at` >= '" + localMonthDate.ToString("yyyy-MM-dd HH:mm:ss") + "' " +
                "   AND `created_at` <= '" + localDate.ToString("yyyy-MM-dd HH:mm:ss") + "' )" +
                " or  (`updated_at` >= '" + localMonthDate.ToString("yyyy-MM-dd HH:mm:ss") + "' " +
                "   AND `updated_at` <= '" + localDate.ToString("yyyy-MM-dd HH:mm:ss") + "' )" +
                ") ";
            shiftUsrEmpQry += " GROUP BY `asm_id`";
            ...
            try
            {
                ...
                // Way 3
                using MySqlCommand shiftUsrEmp = new(shiftUsrEmpQry, conn);
                using MySqlDataAdapter shiftUsrEmpDA = new(shiftUsrEmp);
                shiftUsrEmpDA.SelectCommand.CommandType = CommandType.Text;
                shiftUsrEmpDA.Fill(shiftUsrEmpDS, "Shifts");
            ...
            return shiftUsrEmpDS;
        }
        private DataSet GetAttendanceEmployeeUsers(string asm_id, string dates)
        {
